export class Salable {
  ...
  async init() {
    ...
    if (this.envConfig.apiKey) {
      ...
      let productResponse = {}
      const encoded = encodeURI(`${this.initialisers.getApiDomain()}/products/${this.envConfig.productUuid}/pricingtable?globalGranteeId=${this.envConfig.globalPlanOptions.granteeId}&granteeIds=[${granteeIdsWithHashes}]&globalSuccessUrl=${this.envConfig.globalPlanOptions.successUrl}&successUrls=[${successUrlsWithHashes}]&globalCancelUrl=${this.envConfig.globalPlanOptions.cancelUrl}&cancelUrls=[${cancelUrlsWithHashes}]&member=${this.checkoutConfig.member}${this.initialisers.queryParametersFactory(this.checkoutConfig)}`)
      try {
      ...
      if (productResponse.status === 200) {
        ...
        const plans = productData.plans.filter((p) => p.active && p.planType !== 'bespoke' && p.status === 'ACTIVE').sort((a,b) => {
